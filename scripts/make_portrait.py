...
Run it once; it is not on a schedule, unlike scripts/generate_stats.py.
    ...
    pip install pillow numpy opencv-python-headless          # flat art
    pip install rembg onnxruntime                             # photos only
    python3 scripts/make_portrait.py avatar.png --flat
...
The portrait on this profile is flat vector art, so it takes --flat and none of
the photo machinery: see prep_flat below. The photo path is kept because it is
the better one when the source is a real photograph; its first run downloads a
~176 MB background-removal model, once.
...
FLAT_TOL = 45              # --flat: colour distance that still counts as backdrop
...
def prep(path, crop=None):
    ...
    from rembg import remove          # only the photo path pays for this
...
def prep_flat(path, crop=None, tol=FLAT_TOL):
    """Flat vector art: key out the backdrop, keep the tones the artist chose.

    The photo path above is wrong for an illustration. There is no lighting to
    recover, so CLAHE only turns large flat fills into texture, and rembg is a
    176 MB model asked to find an edge that is already a single hard colour.

    Two things are actually in the way, and both are backdrop: the card colour
    behind the subject, matched here by distance from the colour at the frame's
    centre-top, and the darker page behind the card's rounded corners, which is
    the same value as the hair and so cannot be matched by colour at all. The
    corners are flood-filled from the four corner pixels instead — the subject
    never touches them, so nothing of the drawing is reachable that way.
    """
    src = Image.open(path).convert("RGB")
    if crop:
        src = src.crop(crop)

    rgb = np.array(src).astype("int16")
    h, w = rgb.shape[:2]
    backdrop = rgb[int(h * 0.02), w // 2]             # above the head
    gray = np.array(src.convert("L"))

    gray[np.abs(rgb - backdrop).sum(axis=2) < tol] = 255
    fill = np.zeros((h + 2, w + 2), "uint8")
    for corner in ((0, 0), (w - 1, 0), (0, h - 1), (w - 1, h - 1)):
        cv2.floodFill(gray, fill, corner, 255, 30, 30,
...
def main():
    ...
    ap.add_argument("photo", help="png of the portrait source")
    ...
    ap.add_argument("--flat", action="store_true",
                    help="source is flat vector art, not a photograph: key the "
                         "backdrop out instead of running rembg and CLAHE")
    ...
    prepare = prep_flat if args.flat else prep
    lines = to_lines(prepare(args.photo, crop), cols=args.cols)
    if args.preview:
        print("\n".join(lines))
